...
def ensure_cuda_torch(log=print, force: bool = False) -> bool:
    ...
    try:
        ...
        # (사용자 보고: "재설치 성공했다는데 재시작해도 여전히 CUDA 못 찾음"). 원래
        # --force-reinstall을 썼는데, 이 옵션은 재설치 전에 기존 패키지를 uninstall하려고
        # 시도함 - 포터블 배포판의 torch가 RECORD 파일 없이(정식 pip 설치 경로가 아니게)
        # 깔려있으면 uninstall 자체가 "uninstall-no-record-file" 에러로 실패함(사용자 보고).
        # --ignore-installed는 기존 설치를 건드리지 않고 그냥 덮어써서 uninstall 단계를
        # 아예 안 거치므로 두 문제 다 해결됨. --no-deps를 같이 줘서 torch/torchvision 본체만
